...
- (void)prepareForBodyWithSize:(UInt64)contentLength
{
    ...
    //NSLog(@"파일전체길이: %llu", contentLength);
...
- (void) processContent:(NSData*) data WithHeader:(MultipartMessageHeader*) header 
{
	...
	if( storeFile ) {
        ...
        //NSLog(@"총받은길이:%llu", [storeFile seekToEndOfFile]);
        [[VCWebServ shared] uploadingProcess:self.fileName
                                      totLen:self.fileSize
                                      curLen:[storeFile seekToEndOfFile]];
	}
}
...
- (void) processEndOfPartWithHeader:(MultipartMessageHeader*) header
{
    ...
    //NSLog(@"파일받기 끝");
    [[VCWebServ shared] uploadingComplete:self.fileName totLen:self.fileSize];
}
