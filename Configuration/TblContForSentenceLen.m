...
@interface TblContForSentenceLen ()

@end

@implementation TblContForSentenceLen
...
- (void)viewDidLoad {
    ...
    _arrData = [NSMutableArray arrayWithArray:[Config shared].arrMinSentenceLen];
    
    // SET TITLE
    NSString* naviTitle = [[Config shared] trans:@"한 문장의 최소 시간 길이"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    /*-----------------------
     ...
     ------------------------*/
    float minimumLength = [[Config shared] getMinimumSenSec];
    if (minimumLength == valueFromDataSet.floatValue)
...
{
    NSString* header = [[Config shared] trans:@"한 문장의 최소 시간 길이"];
    return header;
}
...
- (void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // Update config file
    NSString* valueFromDataSet = [_arrData objectAtIndex:indexPath.row];
    [[Config shared] setMinimumSenSec:valueFromDataSet.floatValue];
