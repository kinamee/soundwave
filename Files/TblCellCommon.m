...
- (void)dataFill:(FileInfo*)pFifo
{
    ...
    if ([[Config shared] isSupportedFile:extention])
    {
        ...
        if (pFifo.isTypeOfDir) {
            
            // SET SUB CONTENT COUNT
            NSString* countFile = @"";
            NSString* countDire = @"";
            [[KSPath shared] fileCountInDirectory:pFifo.fileNameFull
                                     refFileCount:&countFile
                                      refDirCount:&countDire];
            UILabel* lblSubCount = [self lableSubCount];
            lblSubCount.text = @(countFile.intValue + countDire.intValue).description;
            
            // SET DETAIL INFO FOR DIRECTORY TYPE
            lblDetail.text = [NSString stringWithFormat:@"Content Count : FOLDER-%@ FILE-%@",
                              countDire, countFile];
            //Content Count : FOLDER-8 & FILE-1
            
            // SET ICON FOR DIRECTORY TYPE
            UIImageView* imageView = [self imageFileIcon];
            [imageView setImage:[UIImage imageNamed:@"icn_dirtype"]];
            imageView.image = [imageView.image imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
            imageView.tintColor = [UIColor colorWithRed:0.263 green:0.263 blue:0.263 alpha:1.00];
            
            // SET ACCESSORY FOR DIRECTORY
